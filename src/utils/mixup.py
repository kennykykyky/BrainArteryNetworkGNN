...
def mixup(batch_data, device='cuda'):
    ...
    x = x.reshape((bz, -1, x.shape[-1])).to(device)
    edge_attr = edge_attr.reshape((bz, x.shape[1], -1)).to(device)
    y = y.to(device)

    mixed_x, mixed_nodes, y_a, y_b, lam = mixup_data(x, edge_attr, y, device=device)
