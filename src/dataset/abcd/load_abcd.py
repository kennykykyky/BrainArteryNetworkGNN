...
def load_data(brave_path):
    file_name = brave_path.split('/')[-1]
    data = np.load(brave_path + f'/{file_name}.npy', allow_pickle=True).item()
    final_pearson = data["connectivity"]
    labels = data["label"]
    edge_features = data["edge_features"]
    node_features = data["node_coordinates"]

    return final_pearson, edge_features, node_features, labels
